...
public class Duck extends Herbivore {
    private static final AnimalConfig config = loadAnimalConfig("Duck");

    public Duck() {
        super(config.getWeight(), config.getMaxAmountPerCell(), config.getSpeed(), config.getRequiredFood());
    }

    public Duck(double weight, int maxAmountPerCell, int speed, double requiredFood, List<Animal> parents) {
