...
public class Rabbit extends Herbivore {
    private static final AnimalConfig config = loadAnimalConfig("Rabbit");

    public Rabbit() {
        super(config.getWeight(), config.getMaxAmountPerCell(), config.getSpeed(), config.getRequiredFood());
    }

    public Rabbit(double weight, int maxAmountPerCell, int speed, double requiredFood, List<Animal> parents) {
