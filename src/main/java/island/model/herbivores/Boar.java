...
public class Boar extends Herbivore {
    private static final AnimalConfig config = loadAnimalConfig("Boar");

    public Boar() {
        super(config.getWeight(), config.getMaxAmountPerCell(), config.getSpeed(), config.getRequiredFood());
    }

    public Boar(double weight, int maxAmountPerCell, int speed, double requiredFood, List<Animal> parents) {
