...
public class Buffalo extends Herbivore {
    private static final AnimalConfig config = loadAnimalConfig("Buffalo");

    public Buffalo() {
        super(config.getWeight(), config.getMaxAmountPerCell(), config.getSpeed(), config.getRequiredFood());
    }

    public Buffalo(double weight, int maxAmountPerCell, int speed, double requiredFood, List<Animal> parents) {
